...
移除默认web框架，默认是启动命令框架，自主选择接入web/wailsApp等
...
- [ ] app : Folder 结构化?
- [ ] config : 初始化 vipper map，其他模块需要再填充进去并刷新获取，当文件变动时也刷新配置
  - [ ] 入参1：是否ENV 
  - [ ] 提供 .env 的环境配置融合处理
  - [ ] 默认的配置文件config ，其他的相当于一级配置
  - [ ] 开放挂载配置入参，挂载则刷新配置，需要替换源 env(key)

- [ ] 挂载目录
- [ ] 挂载命令


分离 加个简单加密存储的配置文件 SQLite ？
