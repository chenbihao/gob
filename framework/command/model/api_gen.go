...
func (gen *ApiGenerator) GenRouterFile(ctx context.Context, file string) error {
	...
	f.Func().Id(tableApi+"Register").Params(jen.Id("r").Op("*").Qual("github.com/chenbihao/gob/framework/gin",
		"Engine")).Error().Block(
...
func (gen *ApiGenerator) GenApiCreateFile(ctx context.Context, file string) error {
 ...
 @Router /${table}/create [post]`
	...
	f.Func().Params(
		...
		jen.Id("c").Op("*").Qual("github.com/chenbihao/gob/framework/gin", "Context"),
	).Block(
		...
		jen.If(
			...
			jen.Id("c").Dot("JSON").Call(jen.Lit(400), jen.Op("&").Qual("github.com/chenbihao/gob/framework/gin", "H").Values(jen.Dict{
		...
		jen.Id("gormService").Op(":=").Id("c").Dot("MustMake").Call(jen.Qual("github."+
			"com/chenbihao/gob/framework/contract", "ORMKey")).Assert(jen.Qual("github.com/chenbihao/gob/framework/contract", "ORMService")),
		// db, err := gormService.GetDB(orm.WithConfigPath("database.default"))
		jen.Id("db").Op(",").Id("err").Op("=").Id("gormService").Dot("GetDB").Call(jen.Qual("github.com/chenbihao/gob/framework/provider/orm", "WithConfigPath").Call(jen.Lit("database.default"))),
		...
		jen.If(
			...
			jen.Id("c").Dot("JSON").Call(jen.Lit(500), jen.Op("&").Qual("github.com/chenbihao/gob/framework/gin", "H").Values(jen.Dict{
...
func (gen *ApiGenerator) GenApiDeleteFile(ctx context.Context, file string) error {
 ...
 @Router /${table}/delete [post]`
	...
	f.Func().Params(
		...
		jen.Id("logger").Op(":=").Id("c").Dot("MustMakeLog").Call(),
		jen.Id("gormService").Op(":=").Id("c").Dot("MustMake").Call(jen.Qual("github.com/chenbihao/gob/framework/contract", "ORMKey")).Assert(jen.Qual("github.com/chenbihao/gob/framework/contract", "ORMService")),
		jen.Id("db").Op(",").Id("err").Op(":=").Id("gormService").Dot("GetDB").Call(jen.Qual("github."+
			"com/chenbihao/gob/framework/provider/orm", "WithConfigPath").Call(jen.Lit("database.default"))),
		jen.If(jen.Err().Op("!=").Nil().Block(
			jen.Id("logger").Dot("Error").Call(jen.Id("c"), jen.Err().Dot("Error").Call(), jen.Nil()),
			jen.Id("_").Op("=").Id("c").Dot("AbortWithError").Call(jen.Lit(50001), jen.Err()),
			jen.Return(),
		...
		jen.If(
			...
			jen.If(
				...
				jen.Id("c").Dot("JSON").Call(jen.Lit(404), jen.Op("&").Qual("github.com/chenbihao/gob/framework/gin", "H").Values(jen.Dict{
...
func (gen *ApiGenerator) GenApiListFile(ctx context.Context, file string) error {
 ...
 @Router /${table}/list [get]`
	...
	f.Comment(comment)
	f.Func().Params(jen.Id("api").Op("*").Id(tableApi)).Id("List").Params(jen.Id("c").Op("*").Qual("github.com/chenbihao/gob/framework/gin", "Context")).Block(
...
func (gen *ApiGenerator) GenApiShowFile(ctx context.Context, file string) error {
 ...
 @Router /${table}/{id} [get]`
	...
	f.Comment(comment)
	f.Func().Params(jen.Id("api").Op("*").Id(tableApi)).Id("Show").Params(jen.Id("c").Op("*").Qual("github.com/chenbihao/gob/framework/gin", "Context")).Block(
...
func (gen *ApiGenerator) GenApiUpdateFile(ctx context.Context, file string) error {
 ...
 @Router /${table}/update [post]`
	...
	f.Comment(comment)
	f.Func().Params(jen.Id("api").Op("*").Id(tableApi)).Id("Update").Params(jen.Id("c").Op("*").Qual("github.com/chenbihao/gob/framework/gin", "Context")).Block(
		jen.List(jen.Id("id"), jen.Id("err")).Op(":=").Qual("strconv", "Atoi").Call(jen.Id("c").Dot("Query").Call(jen.Lit("id"))),
		jen.If(jen.Err().Op("!=").Nil()).Block(
			jen.Id("c.JSON(400, gin.H{\"error\": \"Invalid parameter\"})"),
			jen.Return(),
