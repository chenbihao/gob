...
构建命令，`framework/command/build.go`：
...
交互式输入，并利用官方库 [text/template](https://pkg.go.dev/text/template) 生成对应的代码文件。

>  [survey](https://github.com/AlecAivazis/survey) 于 2023 年 9 月 24 日 停止维护，新推荐 [bubbletea](https://github.com/charmbracelet/bubbletea)

### 代码实现

#### 自动化创建服务工具

`framework/command/provider.go`：
...
挂载相关命令， `framework/command/kernel.go` 增加：
...
# 使用命令来安装 gob 命令。
go install github.com/chenbihao/gob@latest

# 运行初始化脚手架
gob new
# 并根据命令行互动输入对应的应用名与模块名。
...
一类是规则，就是 OpenAPI 的文档，用来规定接口文档的字段含义与要求。

一类是工具，例如 [swagger-ui](https://petstore.swagger.io/)、[swagger-editor](https://editor.swagger.io/) 和 [swagger-codegen](https://swagger.io/tools/swagger-codegen/download/)。

### 如何实现

源码引入通过注释生成 json 与 ui 的开源项目 [swag](https://github.com/swaggo/swag)，注释用法： [说明文档](https://github.com/swaggo/swag#declarative-comments-format)。

使用 [gin-swagger](https://github.com/swaggo/gin-swagger) 中间件来启动 swagger-ui，用法： [例子](https://github.com/swaggo/gin-swagger)。

### 代码实现

#### 引入 swag 与生成命令

源码引入：`framework/middleware/gin-swagge/` ，并且调整 gin 为本项目模块 。

swagger 生成命令，`framework/command/swagger.go`：
...
整个 swagger 接口的说明文档注释， `app/http/swagger.go`：
...
配置文件，`config/dev/app.yaml` 添加：

```go
swagger: true
```

路由注册，`app/http/route.go`：
...
Engine 设置容器相关 `framework/gin/gob_engine.go`：
...
由于在绑定业务层路由内就调用了 `r.GetContainer()`，所以需要把 set 方法行为提前到 `app/http/kernel.go`：
...
命令 `./gob swagger gen` 可以生成 docs 文件。

启动项目后打开 [http://localhost:8080/swagger/index.html](http://localhost:8888/swagger/index.html) 可以看到 swagger-ui 的界面，并且点击 execute 可以调用接口。

## 24、管理进程，完善运行命令

让 app 服务增加管理服务进程的功能，并且命令参数附带配置地址与守护模式参数。

### 如何实现

通过 `app/storage/runtime/app.pid` 存储获取 pid 来管理进程。

同时跟第十四节一样，利用开源库 [go-daemon](https://github.com/sevlyar/go-daemon) 来设置守护进程（win 不支持），利用第三方库 [gspt](https://github.com/erikdubbelboer/gspt) 来设置进程名称（win 也不支持）。
...
同时因为 win 也不支持给进程发信号，所以这里 util 的方法我设置成兼容不同平台的方式来调用：

`framework/util/syscall.go`：
...
这节引入 [Gorm](https://github.com/go-gorm/gorm) 作为框架的 orm 服务，[gorm 文档](https://gorm.io/zh_CN/)。
...
func main() {
	...
	// 插入一条数据
	db.Create(&User{Name: "chenbihao"})
	
	// 相当于： 
	// db, err = sql.Open("mysql", *dsn)
	// result, err := db.ExecContext(ctx, "INSERT INTO user (name) values ('chenbihao')")
}
```

传入数据库连接器 `Dialector` （负责引入驱动 + 创建链接）以及可变长的 `Option`（负责初始化调整配置）。

返回 `gorm.DB` 结构（），一个 `gorm.DB` 结构就代表一个数据库连接，

其中嵌套了 `Config` 结构，包含日志输出 `Logger` 、连接池 `ConnPool` 和 `Callback` 。

```go
// Open initialize db session based on dialector
func Open(dialector Dialector, opts ...Option) (db *DB, err error)

// Dialector GORM database dialectortype Dialector interface {  
    Name() string  
    Initialize(*DB) error  
...
// Option gorm option interfacetype Option interface {  
    Apply(*Config) error  
...
引入 gorm 作为服务提供者，通过读取 `database.yaml` 配置生成 DSN：

- `GetBaseConfig` 获取 `database.yaml` 根目录下的配置，作为默认配置
- `WithConfigPath` 根据指定配置路径获取 Redis 配置
- `WithGormConfig` 可以直接修改 DBConfig 中的 `gorm.Config` 配置信息
...
服务契约 `framework/contract/orm.go`：
...
服务提供者 `framework/provider/orm/provider.go`：
...
服务实例 `framework/provider/orm/service.go`：
...
func (app *GormService) GetDB(option ...contract.DBOption) (*gorm.DB, error) {  
    ...
    // 设置 Logger    ormLogger := NewOrmLogger(logService)  
    ...
    // 如果最终的 config 没有设置 dsn 就生成 dsn    if config.Dsn == "" {  
...
接入日志，`framework/provider/orm/logger.go`：
...
接入 config，`framework/provider/orm/config.go`：：
...
通过读取 `database.yaml` 配置生成 DSN，`config/dev/database.yaml`：
...
`app/http/module/demo/api.go` 的 `Register` 内添加：

```go
r.GET("/demo/orm", api.DemoOrm)
```

`app/http/module/demo/model.go` 添加：
...
`app/http/module/demo/api_orm.go`：
...
先封装 redis 服务，再封装通用的 cache 服务。

### 如何实现

通过 [go-redis](https://github.com/redis/go-redis) 来引入 redis 服务（Redis 6 使用 v8 版本，Redis7 使用 v9 版本，[简中文档](https://redis.uptrace.dev/zh/)）。

服务大体操作跟引入 gorm 差不多，也是通过读取配置来初始化。
...
服务契约 `framework/contract/redis.go`：
...
服务提供者 `framework/provider/redis/provider.go`：
...
服务实例 `framework/provider/redis/service.go`：
...
获取配置 `framework/provider/redis/config.go`：
...
服务契约 `framework/contract/cache.go`：
...
type CacheService interface {  
    ...
    // GetObj 获取某个key对应的对象, 对象必须实现 https://pkg.go.dev/encoding#BinaryUnMarshaler    GetObj(ctx context.Context, key string, model interface{}) error  
    // GetMany 获取某些key对应的值  
    GetMany(ctx context.Context, keys []string) (map[string]string, error)  
  
    // Set 设置某个key和值到缓存，带超时时间  
    Set(ctx context.Context, key string, val string, timeout time.Duration) error  
    // SetObj 设置某个key和对象到缓存, 对象必须实现 https://pkg.go.dev/encoding#BinaryMarshaler    SetObj(ctx context.Context, key string, val interface{}, timeout time.Duration) error  
    ...
    // SetForeverObj 设置某个key和对象到缓存，不带超时时间，对象必须实现 https://pkg.go.dev/encoding#BinaryMarshaler    SetForeverObj(ctx context.Context, key string, val interface{}) error  
...
服务提供者 `framework/provider/cache/provider.go`：
...
服务实现 `framework/provider/cache/services/cache.go`：
...
redis 服务实现 `framework/provider/cache/services/redis.go`：
...
// GetObj 获取某个key对应的对象, 对象必须实现 https://pkg.go.dev/encoding#BinaryUnMarshalerfunc (r *RedisCache) GetObj(ctx context.Context, key string, model interface{}) error {  
...
// SetObj 设置某个key和对象到缓存, 对象必须实现 https://pkg.go.dev/encoding#BinaryMarshalerfunc (r *RedisCache) SetObj(ctx context.Context, key string, val interface{}, timeout time.Duration) error {  
    return r.client.Set(ctx, key, val, timeout).Err()  
}  
...
// SetForeverObj 设置某个key和对象到缓存，不带超时时间，对象必须实现 https://pkg.go.dev/encoding#BinaryMarshalerfunc (r *RedisCache) SetForeverObj(ctx context.Context, key string, val interface{}) error {  
    return r.client.Set(ctx, key, val, NoneDuration).Err()  
}  
...
memory 服务实现 `framework/provider/cache/services/memory.go`：
...
配置文件 `config/dev/cache.yaml`：

```yaml
# driver: memory # 连接驱动（内存缓存）  
driver: redis # 连接驱动（redis缓存）  
...
接口定义 `app/http/module/demo/api_cache.go`：
...
注册接口 `app/http/module/demo/api.go` 添加：
...
运行 app 并访问 `/demo/redis` 与 `/demo/cache` 进行验证

## 28、SSH：发布自动化

框架发布自动化，可以实现开发后直接更新目标服务器应用。

### 如何实现

通过官方的 [ssh](https://golang.org/x/crypto/ssh) 库来连接 ssh，并且创建 deploy 命令，通过第三方库 [sftp](https://github.com/pkg/sftp) 来上传到服务器并执行启动脚本。

### 代码实现

#### 引入 ssh

服务契约，`framework/contract/ssh.go`：
...
服务提供者，`framework/provider/ssh/provider.go` 直接用生成的，延迟加载。

实现，`framework/provider/ssh/service.go`：
...
配置，`framework/provider/ssh/config.go`：
...
命令 `framework/command/deploy.go` ：
