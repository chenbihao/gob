...
引入 gorm 作为服务提供者，通过读取 `database.yaml` 配置生成 DSN，接入日志服务，以单例形式提供服务。
...
`framework/provider/orm/provider.go`：
...
`framework/provider/orm/service.go`：
...
`framework/provider/orm/logger.go`：
